...
void print_chip_report_table(chip_report_table_info *report_entry)
{
	...
	{
		//report_entry->table[entry_idx] = (report_entry->table[entry_idx]) >> 8;
...
*/
	...
	*(tmp_host_16+12) = htons(*(tmp_host_16+12));
	...
	tmp_host_64 = (u64 *)(host+28);//指针指向版本号
	*tmp_host_64 = htonll(*tmp_host_64);//字节序转换
...
void report_table_trans_fun(u32 *tmp_table,chip_report_table_info *chip_report_table)
{
	...
	for(idx = 0;idx<32;idx++)
	{
		...
		//tmp_table[idx] = (u32)((chip_report_table->table[idx])>>8);
	}
...
int cfg_chip_single_register(u16 dimac,chip_reg_info chip_reg)
{
	...
	build_send_chip_cfg_pkt(dimac,CHIP_REPORT_EN_ADDR,1,&cfg_report_en);
	
#if 1
	while(1)
	{
		
		//printf("wait singe reg report pkt\n");
		...
		if(*(pkt+12)==0xff && *(pkt+13)==0x01)
		{
			...
			if(*(pkt+14) == TSMP_BEACON && get_report_type == CHIP_REG_REPORT)
			{
				...
				//8字节为硬件版本寄存器，不需要比较
				if(memcmp(&chip_reg,chip_report_reg,sizeof(chip_reg_info)-8)==0)
				{
...
int cfg_chip_table(u16 dimac,u32 type,chip_cfg_table_info chip_cfg_table)
{
	...
	for(table_idx = 0;table_idx<cfg_num;table_idx++)
	{
		...
		while(1)
		{
			...
			//printf("wait cfg table report pkt\n");
			//printf("cfg type = %x\n",type);
			pkt = data_pkt_receive_dispatch_1(&len);
			if(pkt != NULL) 
			{
			...
			if(*(pkt+12)==0xff && *(pkt+13)==0x01) 
			{
				...
				if(*(pkt+14) == TSMP_BEACON && get_report_type == report)
				{
					...
					if(memcmp((u8 *)table,((u8 *)&(chip_cfg_table.table[table_idx*32])),128)==0)
					{
						printf("cfg type 0x%x,cfg block %d  cfg success\n",type,table_idx);
						printf("999999999999999999999999\n");
						break;
					}
					else
					{
						printf("cfg type 0x%x,cfg block %d  cfg fail\n",type,table_idx);
						//return -1;
					}
				}
			}
