...
u8* build_tsninsight_pkt(u16 pkt_len,u8 type,u16 imac)
{
	u8 *pkt = (u8 *)malloc(pkt_len + sizeof(tsninsight_udp_header)+1);
	bzero(pkt,pkt_len+1);

	//tsninsight的UDP头赋值
	tsninsight_udp_header *udp_header_pkt = (tsninsight_udp_header *)pkt;
	udp_header_pkt->dmac[0] = 0x60;
	udp_header_pkt->dmac[1] = 0x10;
	udp_header_pkt->dmac[2] = 0x00;
	udp_header_pkt->dmac[3] = 0x00;
	udp_header_pkt->dmac[4] = 0x00;
	udp_header_pkt->dmac[5] = 0x00;

	udp_header_pkt->smac[0] = 0x60;
	udp_header_pkt->smac[1] = 0x00;
	udp_header_pkt->smac[2] = 0x00;
	udp_header_pkt->smac[3] = 0x00;
	udp_header_pkt->smac[4] = 0x00;
	udp_header_pkt->smac[5] = 0x00;

	udp_header_pkt->ether_type = ntohs(0x0800);	
	udp_header_pkt->ip_len     = ntohs(pkt_len + sizeof(tsninsight_udp_header));
	udp_header_pkt->src_port   = ntohs(0x9090);
	udp_header_pkt->dst_port   = ntohs(0x9090);

	udp_header_pkt->tsninsight_header_pkt.version = 0x00;
	udp_header_pkt->tsninsight_header_pkt.type    = type;
	udp_header_pkt->tsninsight_header_pkt.length  = ntohs(pkt_len);
	udp_header_pkt->tsninsight_header_pkt.imac    = ntohs(imac);


	//memcpy(pkt,(u8 *)&udp_header_pkt,sizeof(tsninsight_udp_header));

	return pkt+sizeof(tsninsight_udp_header);
}
