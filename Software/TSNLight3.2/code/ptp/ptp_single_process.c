...
u64 sync_seq = 0;                                      //��ʼͬ������Ϊ1
u8 precision;                                       //ͬ��������ֵ
u16 SYNC_TO_INIT_SHRESHOLD = 500;                    //��¼��ǰͬ����ʼ״̬���ʼ״̬ת������ֵ
u16 consequent_sync =0;                                //��¼��ǰͬ������С����ֵ������ͬ������
struct  timeval  tv_sync;                          //��¼�ϴ�ͬ����ϵͳʱ��


//设置每个节点是否接收到报文，-1表示节点不存在，0表示未接收到本轮的同步报文，1表示接收到
u8 get_sync_reply[10] = {0};
u8 get_req_reply[10] = {0};
...
int offset_calu(int value){                            //����imac������Ϣ������offset���м���
   // u32 cfg_offset[2];
	...
	write_offset(value,table.table[value].Latest_Offset);
	if(G_STATE == NW_RUNNING_S)
	{

	
		
		if((table.table[value].Latest_Offset > PRECISION ) && (table.table[value].Latest_Offset <= 50))
		{
			printf("imac=%d node out of precision	 offset = %lld ,sync seq = %lld\n",value,table.table[value].Latest_Offset,sync_seq);
			write_debug_msg("imac=%d node out of precision	  offset = %lld,sync seq = %lld \n",value,table.table[value].Latest_Offset,sync_seq);
			write_debug_msg("t1 = %lld,t2 = %lld,corr_ms = %lld,t3 = %lld,t4 = %lld,corr_sm = %lld \n",
							table.table[value].t1,table.table[value].t2,table.table[value].corr_ms,table.table[value].t3,table.table[value].t4,table.table[value].corr_sm);

			//return 255;
		}
		else if(table.table[value].Latest_Offset > 50)
		{
			printf("state.offset > 50 ! \n");
			printf("imac=%d node out of precision	 offset = %lld ,sync seq = %lld\n",value,table.table[value].Latest_Offset,sync_seq);
			write_debug_msg("imac=%d node out of precision	  offset = %lld,sync seq = %lld \n",value,table.table[value].Latest_Offset,sync_seq);
			write_debug_msg("t1 = %lld,t2 = %lld,corr_ms = %lld,t3 = %lld,t4 = %lld,corr_sm = %lld \n",
							table.table[value].t1,table.table[value].t2,table.table[value].corr_ms,table.table[value].t3,table.table[value].t4,table.table[value].corr_sm);
...
u16 sync_period_timeout(){                                                       //�ж��Ƿ�ͬ����ʱ
    ...
    if(time_interval > ptp_period){
        sync_raw_pkt_send();
		if(G_STATE == NW_RUNNING_S)//如果状态为网络运行状态，则需要判断是否接收到每一轮的ptp报文
		{
			if(sync_count != cur_node_num -1 || req_count != cur_node_num -1)
			{
				write_debug_msg("sync sync seq = %lld  %d %d %d %d %d %d %d %d %d %d\n",sync_seq,get_sync_reply[0],get_sync_reply[1],get_sync_reply[2],get_sync_reply[3],get_sync_reply[4],
																	   get_sync_reply[5],get_sync_reply[6],get_sync_reply[7],get_sync_reply[8],get_sync_reply[9]);
				write_debug_msg("req sync seq = %lld  %d %d %d %d %d %d %d %d %d %d\n",sync_seq,get_req_reply[0],get_req_reply[1],get_req_reply[2],get_req_reply[3],get_req_reply[4],
																	   get_req_reply[5],get_req_reply[6],get_req_reply[7],get_req_reply[8],get_req_reply[9]);	
			}
			memset(get_sync_reply,0,10);//数组清零
			memset(get_req_reply,0,10);//数组清零
			...
			for(i=0;i<10;i++)
			{
				if(get_sync_reply[i]==0)
				{
					write_debug_msg("none get imac = %d sync\n" ,i);
					
				}
				if(get_req_reply[i]==0)
				{
					write_debug_msg("none get imac = %d req\n" ,i);					
				}
				if(get_sync_reply[i] != -1)//判断该节点是否存在，-1表示不存在，其他值表示存在，如果存在，则每次同步后重新清零
				{
					get_sync_reply[i] = 0;//
					get_req_reply[i]  = 0;//
				}
      ...
      //  printf("tv_sync.tv_sec = %ld , tv_sync.tv_usec =%ld , time_interval = %lld , ptp_period = %d\n" ,tv_sync.tv_sec,tv_sync.tv_usec,time_interval,ptp_period);
		if(time_interval > ptp_period*2)//如果大于同步周期的两倍，则记录文本
			write_debug_msg("tv_sync.tv_sec = %ld , tv_sync.tv_usec =%ld , time_interval = %lld , ptp_period = %d,sync seq = %lld \n" ,tv_sync.tv_sec,tv_sync.tv_usec,time_interval,ptp_period,sync_seq);
...
int ptp_handle(u16 pkt_len,const unsigned char *packet_content)
{
   // return;
   int ret = 0;
	...
	//printf("get imac = %d ptp pkt,ptp_type = %d\n",imac,pkt->ptp_type);

	/*��ʱpkt��pkt_len�Ѿ�������tsmpͷ,ΪPTP����*/
	if(pkt->ptp_type == 0x1 && pkt->sync_seq == sync_seq-1){
		
		get_sync_reply[imac] = 1;//1表示接收到本轮的sync报文
		sync_count++;
		...
		get_req_reply[imac] = 1;//1表示接收到本轮的req报文
		req_count++;
		
	    ret = offset_calu(imac);                         //��offsetֵ���м���
		if(ret == -1)
		{
			return -1;
		}
	    ...
	    if(G_STATE == SYNC_INIT_S){
            ...
            if(is_precision_in_shreshold()==1 && consequent_sync>200){                  //�ж�ϵͳͬ�������Ƿ�С����ֵ
            G_STATE = NW_RUNNING_S;                       //��ת����������״̬
			printf("*************enter NW_RUNNING_S********************\n");
			//构建和发送状态上报报文
			/*
			...
			reg_report_info->cur_state	= ntohs(0x01);//基础配置状态
			reg_report_info->state_info = ntohs(0x01);//基础配置成功
			...
			*/
			send_start_report_tsninsight(3,1,0);//时间同步状态，同步成功，imac配置默认值0

			
            u32 cfg_data  = 3;
			u32 cfg_state = 4;
            printf("11cfg finish %d\n",cfg_data);
            build_send_chip_cfg_pkt(ptp_master_imac,CHIP_CFG_FINISH_ADDR,1,(u32 *)&cfg_data);//配置端口类型
            build_send_chip_cfg_pkt(ptp_master_imac,CHIP_CFG_STATE_ADDR,1,(u32 *)&cfg_state);//配置端口类型

			for(int i =0;i<101;i++){
                if(table.table[i].sync_seq!=0){
                     build_send_chip_cfg_pkt(i,CHIP_CFG_FINISH_ADDR,1,(u32 *)&cfg_data);//配置端口类型
                     build_send_chip_cfg_pkt(i,CHIP_CFG_STATE_ADDR,1,(u32 *)&cfg_state);//同步完成，配置状态为3
		...
		imac = sync_reply_stamped_handler(pkt); 
        printf("sync seq erro! \n");
        printf("sync seq = %lld  , pkt seq = %lld  \n",sync_seq,pkt->sync_seq);
		write_debug_msg("imac = %d pkt->ptp_type == 0x3 sync seq erro! sync seq = %lld  , pkt seq = %lld\n",imac,sync_seq,pkt->sync_seq);
	}else if(pkt->ptp_type == 0x1 && pkt->sync_seq != sync_seq-1){

        printf("sync seq erro! \n");
        printf("sync seq = %lld  , pkt seq = %lld  \n",sync_seq,pkt->sync_seq);
		write_debug_msg("imac = %d pkt->ptp_type == 0x1 sync seq erro! sync seq = %lld  , pkt seq = %lld\n",imac,sync_seq,pkt->sync_seq);
	}
