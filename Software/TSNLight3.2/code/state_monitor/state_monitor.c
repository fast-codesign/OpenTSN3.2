...
#define MONITOR_TIMEOUT 10000000 // 10s
...
void write_offset(u16 imac,u64 offset)
{
...
int net_report_cfg()
{
	...
	for(node_idx=0;node_idx<50;node_idx++)
	{
...
int send_sw_run_report_tsninsight(u8 *recv_pkt,u16 imac)
{
	#if 1
	u8 *pkt = NULL;
	u16 *pkt1 = NULL;
	...
	u64 tmp_offset = 0;
	//构建和发送状态上报报文
	//printf("send_sw_run_report_tsninsight\n");
	chip_pkt_static_report *tsninsight_static_report = NULL;

	//对各个接口的计数器进行赋值
	pkt = (u8 *)build_tsninsight_pkt(sizeof(chip_pkt_static_report)+16,0x21,0);//16表示8字节offset，8字节注入提交上溢下溢
	//memcpy((u8 *)(pkt+8),(u8 *)&(chip[imac].state.chip_report.chip_pkt_static),sizeof(chip_pkt_static_report));
	memcpy((u8 *)(pkt+8),recv_pkt,sizeof(chip_pkt_static_report));

	//对offset进行赋值
	pkt2  = (u64 *)pkt;	
	tmp_offset = htobe64(chip[imac].state.offset);
	memcpy(pkt2,&tmp_offset,8);
	//*pkt2 = ntohll(chip[imac].state.offset);
	//printf("chip[imac].state.offset = %lld\n",chip[imac].state.offset);
	//printf("*pkt2 = %lld\n",tmp_offset            );

	//对注入提交的上溢和下溢赋值
	pkt1 = (u16 *)pkt;
	pkt1 = pkt1 + sizeof(chip_pkt_static_report)/2 + 8/2;//指向末尾
	*pkt1     = ntohs(chip[imac].state.chip_report.chip_module_status.ts_inj_underflow_error_cnt);
	*(pkt1+1) = ntohs(chip[imac].state.chip_report.chip_module_status.ts_inj_overflow_error_cnt);
	*(pkt1+2) = ntohs(chip[imac].state.chip_report.chip_module_status.ts_sub_underflow_error_cnt);
	*(pkt1+3) = ntohs(chip[imac].state.chip_report.chip_module_status.ts_sub_overflow_error_cnt);
	...
	*pkt3 = ntohs(imac);
	...
	lo_data_pkt_send_handle((u8 *)pkt,sizeof(chip_pkt_static_report)+16);
...
void state_monitor(u16 pkt_len, const unsigned char *packet_content)
{
	...
	if(report_type == CHIP_STATE_REPORT)
	{
		...
		send_sw_run_report_tsninsight((u8*)packet_content+16,imac);
...
struct  timeval tv_monitor_timeout;

int state_monitor_timeout(struct timeval tv)
{
	...
	if (is_first_pkt == 0)
	{
		...
		for (int i = 0; i < 50; i++)
		{
			tv_node[i] = tv_current;
		}
		report_tv = tv_current;
		tv_monitor_timeout = tv_current;
		is_first_pkt = 1;
		return 0;
	}
	...
	time_interval = (tv_current.tv_sec - tv_monitor_timeout.tv_sec)*1000000 + (tv_current.tv_usec - tv_monitor_timeout.tv_usec);

	if(time_interval > MONITOR_TIMEOUT)//判断当前时间间隔是否超过定时时间
	{
		tv_monitor_timeout = tv_current;
		...
		for(node_idx=0;node_idx < 50;node_idx++)
		{
			if(chip[node_idx].valid == 1)//说明该节点有效
			{
				...
				if(time_interval > NO_PKT_TIMEOUT)
				{
					...
					offline_node_cnt ++;
					write_debug_msg("imac=%d offline，time intervel = %lld \n",chip[node_idx].imac,time_interval);
					return -1;
					//printf("******************************time intervel  %lld\n",time_interval);
					//printf("imac=%d node offline\n",chip[node_idx].imac);
				}
				/*
				if((chip[node_idx].state.offset > PRECISION ) && (chip[node_idx].state.offset <= 50))
				{
					printf("imac=%d node out of precision    offset = %lld ,sync seq = %lld\n",chip[node_idx].imac,chip[node_idx].state.offset,sync_seq);
					write_debug_msg("imac=%d node out of precision    offset = %lld,sync seq = %lld \n",chip[node_idx].imac,chip[node_idx].state.offset,sync_seq);
					//return 255;
				}
				else if(chip[node_idx].state.offset > 50)
				{
