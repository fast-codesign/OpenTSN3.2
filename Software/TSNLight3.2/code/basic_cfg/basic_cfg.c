...
int init_cfg_fun(u8 node_idx)
{
	...
	*/
	...
	printf("init_cfg[node_idx].port_num %d\n",init_cfg[node_idx].port_num);
	...
	for(flowid_idx=0;flowid_idx<init_cfg[node_idx].forward_table_num;flowid_idx++)
	{
		...
		build_send_chip_cfg_pkt(init_cfg[node_idx].imac,CHIP_REPORT_TYPE_ADDR,1,(u32 *)&report);//配置上报类型为转发表的上报类型
	...
	if(init_cfg[node_idx].reg_data.qbv_or_qch == 0)
	{
		...
		if(init_cfg[node_idx].port_num == 1)
		{
			cfg_chip_table(init_cfg[node_idx].imac,CHIP_QGC1_BASE_ADDR,chip_cfg_table);
			//printf("899999999999999999999999\n");
		}
		else if(init_cfg[node_idx].port_num == 4)
		{
