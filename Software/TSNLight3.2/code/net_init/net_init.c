...
void parse_register_xml( xmlNodePtr cur)
{
	...
	{  
		...
		*/
		...
		{
			value=xmlNodeGetContent(entry);
			init_cfg[cur_node_num].reg_data.rc_regulation_value = atoi(value)-22;
			init_cfg[cur_node_num].reg_data.be_regulation_value = atoi(value);//BE比RC小100个bufid
			init_cfg[cur_node_num].reg_data.umap_regulation_value = atoi(value)-8;
			//printf("ST_buf_threshold %d\n",init_cfg[cur_node_num].reg_data.umap_regulation_value);
		}
		if(xmlStrcasecmp(entry->name,(const xmlChar *)"port_num")==0)
		{
			value=xmlNodeGetContent(entry);
			init_cfg[cur_node_num].port_num = atoi(value);
		}
...
void parse_test_xml(xmlNodePtr cur)
{
	xmlNodePtr entry;
	xmlChar* value;
	...
	for(entry=cur->children;entry;entry=entry->next)
    {
		...
		{
			value=xmlNodeGetContent(entry);
			init_test[cur_test_num].imac = atoi(value);
			
			printf("imac %d\n",init_test[cur_test_num].imac);
		...
		{
			value=xmlNodeGetContent(entry);
			init_test[cur_test_num].time_slot = atoi(value);
			
			printf("time_slot %d\n",init_test[cur_test_num].time_slot);
			
		}
		if(xmlStrcasecmp(entry->name,(const xmlChar *)"inject_slot_period")==0)
		{
			value=xmlNodeGetContent(entry);
			init_test[cur_test_num].inject_slot_period = atoi(value);
		
			printf("inject_slot_period %d\n",init_test[cur_test_num].inject_slot_period);
...
/*解析测试仪*/
static void  parse_test_info(xmlNodePtr cur) 
{
   xmlChar* value;
   xmlNodePtr entry;
   cur=cur->xmlChildrenNode;
   while(cur != NULL)
   	{
       //printf("jiexi while take effect\n");
	   if(!xmlStrcmp(cur->name, (const xmlChar *)"register"))
	   	{
         //printf("info test2 take effect\n");
		 parse_test_xml(cur);		 
	   }	   
	   cur = cur->next;    
   }
...
{  
	...
	{  
		...
		else if(!xmlStrcmp(cur->name, (const xmlChar *)"test"))
		{
             printf("doc else if take effect\n");
			 parse_test_info(cur);
			 cur_test_num++;
		}
...
int net_init(u8 *network_inetrface)
{
	...
	//初始化时间同步
	ret = ptp_init(SYNC_PERIOD,SYNC_FLOWID,MASTER_IMAC,DEFAULT_PERCISION);
	if(ret == -1)
	{
	...
	//构建和发送状态上报报文
	printf("net init report\n");
	send_start_report_tsninsight(0,1,0);//初始状态，初始状态成功，imac配置默认值0
