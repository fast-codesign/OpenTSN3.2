...
{  
	...
	cfg_data = 3;//1表示基础配置状态、2表示本地配置状态，3表示时间同步状态，4表示网络运行状态
	build_send_chip_cfg_pkt(imac,
						CHIP_CFG_STATE_ADDR,
						1,
						(u32 *)&cfg_data);
...
int local_cfg(u8 *pkt,u16 pkt_len)
{
	...
	reg_report_info->cur_state	= ntohs(0x02);//本地配置状态
	reg_report_info->state_info = ntohs(0x01);//本地配置成功
	...
	*/
	send_start_report_tsninsight(2,1,0);//本地配置状态，本地配置状态成功，imac填充默认值0
