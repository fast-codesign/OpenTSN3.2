...
u16 sync_period_timeout(){                                                       //�ж��Ƿ�ͬ����ʱ
    ...
    if(time_interval > ptp_period){
		...
		{
			if(sync_count != cur_node_num || req_count != cur_node_num)
			{
				write_debug_msg("sync %d %d %d %d %d %d %d %d %d %d\n",get_sync_reply[0],get_sync_reply[1],get_sync_reply[2],get_sync_reply[3],get_sync_reply[4],
																	   get_sync_reply[5],get_sync_reply[6],get_sync_reply[7],get_sync_reply[8],get_sync_reply[9]);
				write_debug_msg("req  %d %d %d %d %d %d %d %d %d %d\n",get_req_reply[0],get_req_reply[1],get_req_reply[2],get_req_reply[3],get_req_reply[4],
		...
		if(time_interval > ptp_period*2)//如果大于同步周期的两倍，则记录文本
			write_debug_msg("tv_sync.tv_sec = %ld , tv_sync.tv_usec =%ld , time_interval = %lld , ptp_period = %d\n" ,tv_sync.tv_sec,tv_sync.tv_usec,time_interval,ptp_period);
...
void ptp_handle(u16 pkt_len,const unsigned char *packet_content)
{
   // return;
	...
	if(pkt->ptp_type == 0x1 && pkt->sync_seq == sync_seq-1){
		...
		write_debug_msg("imac = %d sync seq erro! sync seq = %lld  , pkt seq = %lld\n",imac,sync_seq,pkt->sync_seq);
	}
